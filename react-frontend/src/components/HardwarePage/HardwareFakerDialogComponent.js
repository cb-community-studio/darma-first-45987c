...
import hardwareFakerFactory from "./hardwareFakerFactory";
...
const HardwareFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = hardwareFakerFactory(fakerCount);
        let results = [];
        setLoading(true);
        try {
            for (let i = 0; i < fakeData.length; i++) {
                const _data = fakeData[i];
                const res = await client.service("hardware").create(_data);
                // const res = _data;
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(HardwareFakerDialogComponent);
